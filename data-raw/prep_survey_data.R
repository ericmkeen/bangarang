...
if(FALSE){
  ...
  write.csv(segments, file='data-raw/bangarang_segments.csv',quote=FALSE,row.names=FALSE)
  write.csv(sightings, file='data-raw/bangarang_sightings.csv',quote=FALSE,row.names=FALSE)
  
  segments_5km <- read_csv('data-raw/bangarang_segments.csv')
  usethis::use_data(segments_5km, overwrite = TRUE)
  
  whale_sightings <- read_csv('data-raw/bangarang_sightings.csv')
  usethis::use_data(whale_sightings, overwrite = TRUE)
  
}

################################################################################
################################################################################
# Add a few more details to segments
...
if(FALSE){
  ...
  segments_5km %>% as.data.frame %>% head
  
  # Add circuit ID, based on table 1-2 (pg 100 of PDF) in Keen 2017 thesis
  circuits <-
    gsheet::gsheet2tbl('https://docs.google.com/spreadsheets/d/14gyUQh3KgLUho7teMz50AsxQ2-Q94SoAdfZSqOqwH-c/edit?usp=sharing') %>%
    mutate(begin = lubridate::force_tz(lubridate::as_datetime(begin), 'Canada/Pacific')) %>%
    mutate(end = lubridate::force_tz(lubridate::as_datetime(end), 'Canada/Pacific')) %>%
    rowwise %>%
    mutate(middate = mean(c(begin, end))) %>%
    mutate(middate_yday = lubridate::yday(middate)) %>%
    ungroup
  
  circuits %>% head
  
  segments_5km %>% head
  segments_5km$datetime
  segments_5km$circuit <- NA
  segments_5km$circuit_name <- NA
  i=2
  for(i in 1:nrow(segments_5km)){
    (segi <- segments_5km[i, ])
    (matchi <- (which(circuits$begin <= segi$datetime &
                        circuits$end >= segi$datetime)))
    (circi <- circuits[matchi,])
    segments_5km$circuit[i] <- circi$circuit
    segments_5km$circuit_name[i] <- circi$name
    segments_5km$circuit_yday[i] <- circi$middate_yday
  }
  segments_5km$circuit
  segments_5km$yday
  segments_5km <-
    segments_5km %>%
    mutate(circuit_name = factor(circuit_name,
                                 levels=circuits$name[order(circuits$circuit)]))
  
  segments_5km %>% as.data.frame
  usethis::use_data(segments_5km, overwrite = TRUE)
  usethis::use_data(circuits, overwrite = TRUE)
}
