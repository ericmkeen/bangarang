...
This vignette was made with `bangarang` version `r utils::packageVersion('bangarang')`, and will make use of a few other packages: 
...
```{r predict_circuits_1, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
d_circuits <- 
...
Next, we can use the function `bootstrapper_wrapper()` to repeat the circuit density prediction routine with iteratively resampled versions of the segments and sightings data. (Here we only perform 50 bootstrap iterations, but the norm is typically 1,000 or more -- prepare for an overnight run!).  

```{r bootstrap, echo=TRUE, eval=rerun_from_scratch, warning=FALSE, message=FALSE}
bootstraps <-
...
```{r echo=TRUE, eval=rerun_from_scratch, warning=FALSE, message=FALSE}
save(bootstraps, file='demo_boots.rds')
```
...
Flying birds pose special challenges (see [Spear, Nur & Ainley 1992](https://www.dropbox.com/scl/fi/ilb7uoqa6rb578tk1x8mo/Spear-Ainley_1992_FlyingBirdDensity.pdf?rlkey=h9v9aapm9kt89x7sse70jm94v&dl=0), [Spear & Ainley 1997](https://www.dropbox.com/scl/fi/17gy029cv9eb1371cbh15/Spear-Ainley_1997_FlightnDirection.pdf?rlkey=rypcr8867ol217f5avg5lmpkp&dl=0), and [this white paper](https://www.dropbox.com/scl/fi/e7ewvxxoyyz751ijvwjz1/Seabird-Density-Estimation-20150508.pdf?rlkey=1jgpte93552v6w8rb09dmorwz&dl=0) on handling the issues involved). In order to account for the bias caused by the possibliity of repeat-counting a flying bird, we need to perform corrections that rely upon ship speed and direction; bird speed, direction and flight height; and published regressions that predict bird speed for different seabird groups at certain orientations to the wind and at certain heights above the water. 
...
We then use that regression data in the function `calculate_flight_flux()`, which scales group size by the flux correction factor (see [this white paper](https://www.dropbox.com/scl/fi/e7ewvxxoyyz751ijvwjz1/Seabird-Density-Estimation-20150508.pdf?rlkey=1jgpte93552v6w8rb09dmorwz&dl=0) for details):  
