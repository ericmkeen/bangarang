...
That function, `predict_dsm_kfs()`, returns a vector of density estimates equal in length to the number of rows in the `grid` `data.frame`.  

We then use that DSM to predict murrelet density on our spatial grid during each circuit of the study area. We completed a total of 9 circuits whose information is contained in the built-in dataset `data(circuits)`:  
...
dsm_comps$dsm
dsm_comps$objects[[1]] %>% summary
```


```{r echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
################################################################################
# DSM point estimates & bootstraps
...
# Point estimate
point_estimate <-
  predict_circuits(dsm_keep, segments)
point_estimate %>% head
...
```{r echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE}
# Estimate log-normal group size
mamu <- group_size_draw(mamu)

# Note: how to handle uncertain zone boundaries?
