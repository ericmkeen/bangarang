...
```{r echo=FALSE, eval=rerun_from_scratch, warning=FALSE, message=FALSE}
bootsumm <- bootstrapper_summary(bootstraps)
save(bootsumm, file='demo_boots.rds')
```
...
result <- 
  left_join(point_summary(d_circuits) %>% select(-dsd),
          (bootsumm %>% select(circuit_name, SD, L95, U95)),
          by='circuit_name') %>% 
  mutate(CV = SD / dmean)
